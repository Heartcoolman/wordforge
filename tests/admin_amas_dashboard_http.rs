...
fn seed(app: &TestApp) {
    ...
    for i in 0..14 {
        conn.execute(
            ...
            params![format!("w{i}"), format!("word{i}"), "m", (i as f64) / 14.0, now_s],
        )
        .unwrap();
    }
    ...
    for (h, eps) in [("vA", 0.30), ("vB", 0.41)] {
        conn.execute(
            ...
            params![h, format!("{{\"memory_model\":{{\"half_life_base_epsilon\":{eps}}}}}"), now_s],
        )
        .unwrap();
    }
    ...
    type EventRow = (&'static str, &'static str, i64, f64, i64, i64, &'static str, &'static str);
    let events: [EventRow; 12] = [
        ...
        ("e5", "swd", 1, 0.1, 2, 0, "[]", "vA"),
        ("e6", "heuristic", 0, 0.4, 8, 1, r#"[{"field":"fatigue","value":1.2,"expected_range":"[0, 1]"}]"#, "vA"),
        ("e7", "ensemble", 1, 0.2, 3, 0, "[]", "vB"),
        ("e8", "ensemble", 1, 0.5, 4, 0, "[]", "vB"),
        ("e9", "mdm", 1, 0.92, 5, 0, "[]", "vB"),
        ("e10", "mdm", 1, 0.15, 4, 0, "[]", "vB"),
        ("e11", "swd", 0, 0.3, 7, 0, "[]", "vB"),
        ("e12", "heuristic", 0, 0.2, 9, 1, r#"[{"field":"batch_size","value":0,"expected_range":">= 1"}]"#, "vB"),
    ];
...
async fn get_json(app: &axum::Router, token: &str, path: &str) -> serde_json::Value {
    let resp = request(app, Method::GET, path, None, &[("authorization", auth_header(token))]).await;
...
async fn it_amas_dashboard_endpoints_real_aggregation() {
    ...
    // 1. 阶段分布
    let d = get_json(&app.app, &token, "/api/admin/amas/metrics/stage-distribution").await;
    assert_eq!(d["totalUsers"], 6);
    assert_eq!(d["stages"].as_array().unwrap().len(), 3);
    assert!(!d["trend"].as_array().unwrap().is_empty(), "trend 应含当日实时点");
    let cold = d["stages"].as_array().unwrap().iter().find(|s| s["stage"] == "cold").unwrap();
    ...
    assert!(p0.get("elo").is_some() && p0.get("decisions").is_some() && p0.get("deltaElo").is_some());
    assert!((p0["deltaElo"].as_f64().unwrap() - 20.0).abs() < 1e-6, "Δ = 当前-8天前 = 20");

    // 3. MDM 热图
    let d = get_json(&app.app, &token, "/api/admin/amas/metrics/mdm-heatmap?days=7").await;
    assert_eq!(d["bandCount"], 14);
    assert!(!d["days"].as_array().unwrap().is_empty());
    assert!(d["peak"].as_f64().unwrap() > 0.0);

    // 4. 疲劳时序
    let d = get_json(&app.app, &token, "/api/admin/amas/metrics/fatigue-timeseries?days=7").await;
    assert!(d.get("threshold").is_some());
    assert!(d["totalTriggers"].as_u64().unwrap() >= 3, "0.9/0.95/0.92 等应触发");
    assert!(!d["points"].as_array().unwrap().is_empty());

    // 5. 决策直方图
    let d = get_json(&app.app, &token, "/api/admin/amas/metrics/decision-histogram?days=7").await;
    assert_eq!(d["buckets"].as_array().unwrap().len(), 7);
    assert_eq!(d["totalUsers"], 6);

    // 6. 状态流转
    let d = get_json(&app.app, &token, "/api/admin/amas/user-state/transitions?hours=48").await;
    let new_cold = d["transitions"].as_array().unwrap().iter()
        .find(|t| t["from"] == "new" && t["to"] == "cold").unwrap();
    ...
    assert!(d["summary"]["error"].as_u64().unwrap() >= 1, "batch_size → error");
    assert!(d["summary"]["warn"].as_u64().unwrap() >= 1, "fatigue 越界 → warn");
    assert_eq!(d["items"].as_array().unwrap().len(), 2);
    assert!(d["items"][0].get("impactedUsers").is_some(), "异常项应含影响面字段");
    assert!(d["items"][0].get("impactPct").is_some());

    // 9. 版本对比扩展（含 configEpsilon 从 snapshot 读取）
    let d = get_json(&app.app, &token, "/api/admin/amas/compare/ext?versionA=vA&versionB=vB").await;
    ...
    assert!(d["a"].get("p95CompletionMs").is_some(), "应含 P95 答题完成时间字段");
