...
`HLRScheduler` 当前 θ = (2.0, -2.5, -0.3)，导致 5+ correct 后 halflife > 100 天，对真实 lapse 给极低 likelihood，三数据集平均 logLoss = 8.77（远高于其他 algo）。原 Duolingo paper θ ≈ (0.5, -1.0, -0.3)，预计 logLoss 回到 0.5-0.7 区间。
...
duolingo_hlr 的 next_r 正样本率 ≈ 87%（用户答对占绝对多数），8 个 algo 平均 AUC = 0.537（接近随机）。该数据集 prediction 维度区分度极低，但 policy / DHP 维度仍有信号。建议未来在 final_score 公式中对 duolingo_hlr 单独降低 prediction 权重，或仅用 ICI / Brier 校准分量。
