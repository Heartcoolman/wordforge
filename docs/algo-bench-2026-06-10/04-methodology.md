...
AMAS 的 `wordSelector` / `ensemble` 在 prediction 维度不起作用（adapter 在评测层仅注入 MDM），因此 AMAS 与 FSRS 在三数据集上的 `logLoss / ici / auc / maeP` **完全相同**。这与 `docs/superpowers/specs/2026-06-10-amas-algo-comparison-design.md` § 9 YAGNI 结论一致：MDM-only 设计有效，扩展到 wordSelector/ensemble 的 30+ 参数对 next-step recall prediction 无贡献。
...
- DHP scheduler 在 synthetic 上 logLoss = 2.328，反而高于 AMAS / FSRS 的 0.493。
- AUC：DHP = 0.632，AMAS = 0.608 —— DHP 仅略胜。
