...
## [v1.2.0-beta.3] — 2026-06-16 · Beta · 管理后台设计对齐 + 远程探针默认启用

承接 beta.2 蓝玻璃重设计的一轮设计对齐与修复，并把远程探针调整为默认启用。**含一处后端行为变更**（远程探针默认值），其余为 admin GUI（运维面板）调整；版本号 bump（core 1.2.0-beta.2 → 1.2.0-beta.3），无破坏性变更。

### 变更
- **远程探针默认启用**：`PROBE_ENABLED` 生产默认值 false → true（`Config::from_env`）。远程探针是向客户端设备下发 JS 到沙箱执行的能力，仍受 per-admin 限速、全量审计留痕、`ctx` 白名单、storage 值脱敏约束；设 `PROBE_ENABLED=false` 可关闭。`.env.example` 与运维 / 客户端文档同步。
- **新手引导（onboarding）重写**：欢迎屏更新为 v1.2.0 重设计亮点；功能屏由旧 5 屏精简重写为「全新仪表盘 / 玻璃侧栏 7 组导航 / 自动刷新更顺滑」3 屏（共 5 屏）。修复 `waveOf()` 将 1.2.x 误并入旧波次的问题 → 新设 `v1.2-redesign` 波次，看过旧导览的用户升级后重弹一次（同步单测与 e2e 预置值）。
- **远程探针脚本模板 +6**：新增「健康总览 / 控制台日志 Tail / 页面加载时序 / 网络质量 / 时钟·时区 / 应用状态快照」，均为只读、仅用 ctx 白名单 API。

### 修复
- **算法决策分布面板**：副标题写死「各算法占比」→ 动态「N 种算法占比」；0 决策时由孤立的「0」改为规整的「暂无数据」空状态（有数据时 donut + 图例本就对齐设计）。
- **升级历史混入通用 admin 审计**：`update_audit_log` 自 P2.10 起为通用 admin 审计表，资源包热更 / 封禁 / 改密等无版本行混进「升级历史」显示为空白「→」。修复：`list_update_audit` 按 `action IN (self_update, rollback)` 过滤；通用 admin 审计改走 `/users/:id/audit-log` 按目标查看；前端动作列按 `action` 正确区分升级 / 回滚。
- **CHANGELOG 面板**：无 GitHub commits 可解析时原样 dump markdown 源码 → 改为解析 release notes 的 markdown 分节渲染（彩色分节标题 + 列表），对齐设计稿。

### 测试
- admin-ui：`tsc --noEmit` 0 错误、`vitest` 149 文件 / 1435 通过 / 9 跳过、`build` 绿。
- 后端：`update_audit` 单测（含「通用 admin 审计不混入升级历史」回归）+ `admin_extra_http` / `cov_admin_updates_http` / `probe_integration` 等相关 integration 全过。
