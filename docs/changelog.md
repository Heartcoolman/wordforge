...
### 安全
- 代理信任进一步收口（承接 beta.25「代理信任收口」）：开启「信任前置代理」（`trust_proxy`）时，`X-Forwarded-Host` 此前只要开关打开就无条件采信；现须额外命中新增的主机白名单（环境变量 `RESOURCE_PACK_TRUSTED_HOSTS`，逗号分隔）才会用于资源包下载地址，未命中则回退真实连接来源，杜绝伪造该请求头污染资源包下载地址的公共缓存。留空即不采信任何转发主机，等同此前直连场景。
- 管理员登录与普通用户登录的失败次数限流此前共用同一 IP 配额：同一出口 IP（公司网络 / VPN 常见）下大量普通用户登录尝试可能连带耗尽该 IP 下管理员登录的可用配额。现两者各自独立计数，互不挤占。
