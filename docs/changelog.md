...
## [v1.3.0] — 2026-08-12 · Stable · 1.3.0 转正

将经 Beta 阶段（v1.3.0-beta.1 ~ v1.3.0-beta.2）验证的 **1.3.0 版本**转正为正式稳定版。这是自 v1.2.0 以来首个稳定版。

- **内容与 `v1.3.0-beta.2` 完全一致**，仅版本号变更（`1.3.0-beta.2` → `1.3.0`）；无新增功能、无破坏性变更。
- **累积转正范围**：三端统一客户端埋点事件流（`POST /api/telemetry/app-events` 批量摄取 + 日聚合 + 8 个分析读端点 + 独立采样开关）· 学习记录上报默认异步化（`RECORDS_OUTBOX_ASYNC` 默认 `true`，outbox 重试 + 死信兜底）· 管理后台新功能导览 v1.3 波次。逐项明细见下方各 Beta 条目。

### ⚠️ 升级前必读（从 v1.2.0 稳定版升级务必逐条确认）

1. **先升级学习端客户端再升级服务端**：iOS / Android / Web 须 ≥ 1.6.0（已发布）。旧客户端在记录异步受理模式下不崩溃，但每题即时反馈退化为估算/延迟回填。
2. **先手动备份数据库再升级**：Admin「版本更新 → 备份」手动触发一次，或 SSH 备份 `data/learning.db`。
3. **迁移单向不可逆**：本次含 m073 埋点四表迁移，升级成功后库结构前滚至 schema v73；回退仅允许换回旧二进制，**禁止**恢复旧库备份。
4. **异步受理可回退**：如需暂回同步老路，设 `RECORDS_OUTBOX_ASYNC=false`（埋点链路不受该开关影响）。

---

## [v1.3.0-beta.2] — 2026-08-12 · Beta · 管理后台导览对齐 v1.3

### 变更
- 管理后台「新功能导览」更新为 v1.3 波次：欢迎屏介绍端侧埋点数据链路与学习记录异步受理，功能屏直达数据探针（埋点采样）与监控页（outbox/死信）。修复升级到 1.3.x 后导览自动重弹但内容仍是 v1.2.0 旧文案的问题。

### 修复
- 管理后台 dev/e2e 基建对齐生产行为（裸 `/admin` 与根级旧路径的开发服务器兜底），不影响生产部署形态。

---

## [v1.3.0-beta.1] — 2026-08-12 · Beta · 客户端埋点事件流 + 学习记录上报默认异步化（S2 事件总线收尾）

本版新增三端统一的客户端埋点事件流（行为/错误/性能），并完成 v1.1 起承诺的 S2「records→AMAS 事件总线化」最后一步：单条学习记录上报默认走 outbox 异步路径。**跨仓前置条件已满足**：三端学习客户端（iOS / Android / Web ≥ 1.6.0）均已确认容忍无 `amasResult` 的 202 受理响应。

### 新增
- **`POST /api/telemetry/app-events`**：三端统一的命名事件埋点批量摄取（behavior / error / perf 三类，≤50 条/请求，逐条校验部分成功；`(deviceId, clientEventId)` 跨请求幂等，重放安全）。事件词表与字段约束见对接文档 §12.5。
- 埋点分析读端点 **`GET /api/admin/app-events/{overview,trend,top-events,errors,perf,funnel,retention-matrix,activity}`**：事件总量/DAU/错误/崩溃自由率/接口耗时分位、事件排行、错误分组样本、使用漏斗、周留存矩阵、活跃度平台构成；历史日读日聚合，当日实时补齐。
- 新增 worker `app_event_rollup`（每日 01:10 幂等重算最近 7 天日聚合）；埋点 raw 事件保留 90 天（沿用 `TELEMETRY_RETENTION_DAYS`），日聚合长期保留。
- 采样配置（probe sampling）新增 `app_behavior` / `app_perf` 两行，可独立调埋点采样率；错误类事件恒不采样。
- 数据库 schema 升级至 v73（m073，四张 app_events 系表，支持任意版本回滚）。

### 变更
- **`RECORDS_OUTBOX_ASYNC` 默认值 `false` → `true`**：`POST /api/records` 单条上报默认返回 202 受理（裸响应 `{accepted, async, clientRecordId}`，不含 `record`/`amasResult`），AMAS 处理由 outbox worker 异步消费（指数退避重试 + 死信兜底 + `processed_events` 幂等账本防重复应用）。设 `RECORDS_OUTBOX_ASYNC=false` 可回退同步老路（`amasResult` 同步返回）。批量 `/api/records/batch` 不受影响，仍恒为同步路径。
- **升级须知（学习端）**：请先将 iOS / Android / Web 升级到 ≥ 1.6.0 再升级服务端；旧客户端在异步模式下无法读到即时的答题反馈（不崩溃，但每题反馈会退化为估算/延迟回填）。

### 移除
- 移除学习记录写入失败时的手动引擎状态快照回滚（含批内逐条回滚与配套存储原语）：落库失败时 AMAS 状态与幂等标记原样保留（「标记存在 ⟺ AMAS 已应用」不变式不变），重试（outbox worker 退避 / 客户端同 `clientRecordId` 重发）命中幂等账本短路、仅补落记录行。失败恢复语义与崩溃窗口重放统一，消除回滚路径用陈旧快照覆盖并发已提交状态的风险面。

---

## [v1.2.0] — 2026-07-21 · Stable · 1.2.x 大版本转正

将经 Beta 阶段（v1.2.0-beta.1 ~ v1.2.0-beta.29）验证的 **1.2.x 大版本**转正为正式稳定版。这是自 v1.1.4 以来首个稳定版。

- **内容与 `v1.2.0-beta.29` 完全一致**，仅版本号变更（`1.2.0-beta.29` → `1.2.0`）；无新增功能、无破坏性变更。
- **累积转正范围**：AMAS 记忆引擎（FSRS 口径调参 + 学习调度退役机制）· 内置托管学习端 Web + Web 端在线下发 · 管理界面迁至 `/admin` 并完成单色「黑墨白纸」重设计 · 公开健康面板（`/health`）与服务体征页 · Web 设备指纹封禁与关联风控 · 自更新回滚重做（任意版本可回退 + 数据可恢复）· 一批安全加固（登录防枚举 / 代理信任收口 / 版本门控防绕过 / GDPR 导出原子冷却）与并发一致性修复。逐项明细见下方各 Beta 条目。

### ⚠️ 升级前必读（从 v1.1.4 稳定版升级务必逐条确认）

1. **先手动备份数据库再升级**：Admin「版本更新 → 备份」手动触发一次，或 SSH 备份 `data/learning.db`。
2. **迁移单向不可逆**：本次累积多批数据库迁移（含指纹存量清理、监控数据回填），升级成功后库结构前滚至最新；回退仅允许换回旧二进制，**禁止**恢复旧库备份。
3. **管理界面入口变更**：自 beta.22 起管理后台由根路径迁至 **`/admin`**，根路径让位给内置学习端 Web；书签与反代规则需同步更新。
4. **反代部署检查 `RESOURCE_PACK_TRUSTED_HOSTS`**：开启 `trust_proxy` 且依赖转发主机推断资源包下载地址的部署，须设置该白名单（或直接设 `RESOURCE_PACK_BASE_URL`），否则下载地址回退为真实连接来源。

---

## [v1.2.0-beta.29] — 2026-07-21 · Beta · 学习调度退役机制 + 算法船值更新 + 管理后台数据回填与安全收口

本版引入学习调度「退役」机制与第六代算法船值，集中修复管理后台长期为空的记忆内部量与用户状态数据（升级时自动回填历史数据），并进一步收口代理信任与登录限流。**本版含数据库结构升级，首次启动自动迁移。**

### 新增
- 学习调度「退役」机制：已牢固掌握的词（毕业且连续答对达到阈值）自动放入超长复习间隔，显著降低成熟词的复习负担。配置项 `gspRetireAfterReviews` / `gspRetireIntervalDays` / `gspRetireMinStability` / `gspRetireMinStreak`（`amas_config.toml`），置 `gspRetireAfterReviews = 0` 即回退旧行为。

### 变更
- 调度船值更新（2026-07-08 算法评测第六代，真实记忆半衰期口径下 11 算法综合第一）：年轻词目标保持率 `gspYoungRetention` 0.86 → 0.90（新词复习更密，长期巩固率提升）；退役机制默认启用（连击 6 次）。
- 预测读出层 `difficultyLogitWeight` 0.1 → 0.0：关闭词难度加性修正（跨数据集综合为净负收益；预测精度回到 FSRS-6 基准最优档）。待词库难度真实回填并经线上 A/B 验证后再评估重开。

### 修复
- 管理后台用户详情与监控看板的词级记忆内部量（稳定度 / 难度 / 记忆强度 / 复习次数）此前恒为空，现随每条作答落库，升级时自动回填历史数据。**本项含数据库结构升级，首次启动自动迁移。**
- 用户管理页「最近登录」恒显示「从未」、「7 天未登录」筛选恒命中全部用户：登录成功现会记录登录时间（历史登录时间无法追溯，自本版首次登录起生效）。
- 封禁 / 解封用户现联动用户状态（禁用 ↔ 活跃），「活跃」计数与状态筛选不再失真；存量已封禁用户状态在升级时自动修正。
- 监控看板 AMAS 用户状态均值（注意力 / 疲劳 / 动机 / 信心）、认知能力分布与阶段 7 天留存此前恒为默认常数或 0，现随作答同步落库，升级时自动回填历史数据。
- 管理后台用户详情「每日目标」不再恒显示 30 词 / 25 分钟：改读用户学习配置中的真实每日词数；「分钟」无真实数据来源，已移除。
- 资源包上传接口消除并发竞态：极短时间窗口内对同一 (pack_id, version) 发起两次上传，此前可能导致数据库记录的校验值与实际落盘文件不一致；现改为串行处理，第二次上传按既有的「版本已存在」规则正常拒绝。

### 安全
- 代理信任进一步收口（承接 beta.25「代理信任收口」）：开启「信任前置代理」（`trust_proxy`）时，`X-Forwarded-Host` 此前只要开关打开就无条件采信；现须额外命中新增的主机白名单（环境变量 `RESOURCE_PACK_TRUSTED_HOSTS`，逗号分隔）才会用于资源包下载地址，未命中则回退真实连接来源，杜绝伪造该请求头污染资源包下载地址的公共缓存。留空即不采信任何转发主机，等同此前直连场景。**升级动作**：反代部署此前依赖转发主机推断资源包下载地址的，升级后须设置 `RESOURCE_PACK_TRUSTED_HOSTS=<对外域名>`（或直接设 `RESOURCE_PACK_BASE_URL`），否则下载地址会回退为真实连接来源的 http 地址；配置缺失时启动日志会给出提示。
- 管理员登录与普通用户登录的「登录接口按 IP 请求次数限流」此前共用同一配额：同一出口 IP（公司网络 / VPN 常见）下大量普通用户登录请求可能连带耗尽该 IP 下管理员登录的可用配额。现两者各自独立计数，互不挤占。

### 移除
- 清理一批从未产生真实数据的字段（升级时自动迁移，各端向后兼容）：用户注册来源、ELO 不确定度、词向量（含语义搜索的向量检索基建，接口行为不变，仍为关键词匹配）、词源生成时间、遥测全局采样率数据库配置（固定为全采样，实际采样由采样规则控制）、遗忘风险接口的上期对比占位块，及若干仅存于数据库的冗余列。

### 本轮深度审查修复汇总（服务端）
- 管理后台实时通道独立化：管理员事件走独立推送通道与独立连接配额，用户侧连接高峰不再挤占管理员观测通道，反之亦然。
- 强制升级「解除」信号补充来源标识：客户端可区分定向解除与全局解除，定向解除不再被误当成全局放行。
- 学习记录写入回滚闭环：同一用户并发提交时的失败回滚不再可能用陈旧状态覆盖已提交数据（含全局词难度评分）；单条与批量共用同一原子回滚路径。
- 学习引擎全局词难度评分（ELO）写入门控显式化：诊断 / 校准类接口不再计入全局评分，仅正规学习流驱动。
- 数据库恢复与手动备份不再受客户端断开影响：请求中断只丢响应，恢复流程、维护模式复位与审计记录照常完成。
- 客户端契约校验（strict-mode）误伤回退：声明 Web 平台但不发送 Sec-Fetch 头的老浏览器不再被最低版本门控误拒；该信号降级为观测计数，供评估误伤率与伪造率。
- Web 设备指纹存量清理：指纹算法调整（移除屏幕缩放维度）后旧指纹哈希已失效，升级时一次性清空，设备下次上报自动按新算法重建；升级后首次启动自动迁移。
- 资源包管理加固：Web 应用版本的删除保护与切换激活互斥、通用资源包激活补齐「已下架版本拒绝」与通道校验盲区、上传失败自动清理残留文件、转发主机白名单启动时归一化并对无效条目告警。

---

## [v1.2.0-beta.28] — 2026-06-30 · Beta · Web 端下发 + 公开健康面板 + 监控看板服务端支撑

本版新增 Web 端在线下发与发版页、公开可视化健康面板，并为监控看板补齐日志查看与监控数据端点。**本版含数据库结构升级，首次启动自动迁移**，不涉及已有对外接口的破坏性变化。

### 新增
- Web 端在线下发：管理后台新增「Web 发版」页（`/admin/web-release`），支持上传 Web 构建包并发布 / 回滚 / 停用；线上 Web 版本以 `status.webTargetVersion` 为单一真相源。
- 公开健康监控面板：用浏览器访问 `/health` 返回可视化体征面板；机器与负载均衡探针仍返回原 JSON，向后兼容。
- 监控看板数据面：日志查看器（按条件过滤 + 实时订阅 + 归档下载）与多个只读监控端点（选词拒绝原因、词态迁移、决策追踪、Worker SLO、ELO 贡献、指纹碰撞等）。

### 变更
- 日志归档支持磁盘占用裁剪：可用 `MAX_LOG_FILES`（默认 30）配置保留的归档文件数上限。
- 细粒度监控数据默认不再降采样，管理端可按需拉取原始口径（`?raw`）。

### 修复
- 健康面板时钟行：时钟漂移时该行状态未能恒绿的显示问题已修。

## [v1.2.0-beta.27] — 2026-06-24 · Beta · 管理后台单色视觉收口

承接 beta.26 的管理后台单色重设计，清理残留的非单色细节，使全后台视觉与设计系统完全一致。升级后数据库无结构变更，不涉及对外接口变化。

### 变更
- 管理后台进度条、开关、分页、按钮与主卡片标题去除渐变填充，统一为单色实色；个别图标位的 emoji 字符替换为线描图标，与「黑墨白纸」视觉规范保持一致。

## [v1.2.0-beta.26] — 2026-06-22 · Beta · 管理后台单色重设计 + 服务体征页 + 设备封禁可靠性修复

本版以管理后台整体「单色重设计」为头条，并补齐服务体征页；同时集中修复设备封禁/解封的可靠性、被封设备的实时通道隔离、强制升级广播的覆盖完整性，并补齐设备封禁类操作的审计留痕。升级后数据库无结构变更，不涉及对外接口变化。

### 变更
- 管理后台整体改为单色「黑墨白纸」视觉：以 1px 发丝线与排版层次取代色块与玻璃质感，主色为墨黑；仅学习状态（新词 / 学习中 / 复习中 / 已掌握 / 已遗忘）保留语义配色。提供亮色 / 暗色两套主题，覆盖全部管理页面及登录、初始化页。

### 修复
- 封禁/解封设备不再「提示失败但实际已生效」：封禁与关联设备风控打标、解封与关联标记清理改为单事务原子完成，关联步骤偶发失败不再让整个操作返回「服务器内部错误」而封禁却已落库。
- 关联风控标记纠错：当某设备同时被多台被封设备牵连时，解封其中一台不再误清或漏清该设备的标记——仅当它不再与任何仍被封设备共享出口 IP / 账号 / 设备指纹时才解除标记。
- 强制升级广播覆盖完整性：受众改为一次性一致快照计算，修复设备列表在扫描期间因活跃刷新导致部分目标设备漏推 / 重复推送强制升级。
- 仅经遥测心跳上报的 Web 设备现也会落库浏览器指纹，使「换设备 ID 仍被封」的指纹封禁与模糊指纹关联打标对这类设备同样生效。

### 安全
- 被封设备实时通道隔离：设备在连接存活期间被封后，服务端立即停止经该长连接下发学习状态、维护、更新等业务推送（仅保留封禁 / 解封信号），防止被封设备（含忽略封禁事件的改装客户端）继续借已建立连接窃取实时状态。

### 新增
- 设备封禁、解封、清除关联标记三类管理操作纳入管理审计日志，可事后追溯操作人、时间与理由。
- 管理后台新增「服务体征」页（路径 `/admin/vitals`，「运维监控」导航组与 ⌘K 命令面板可达）：以实时脉冲波形配合 正常 / 降级 / 故障 三态联动，一屏展示服务整体健康、可用性、运行时长、数据库体积及各子服务（实时通道 SSE、时钟对齐、词库中心等）状态；数据源为公开健康端点 `/health`，进入页面后自动刷新。

## [v1.2.0-beta.25] — 2026-06-21 · Beta · 安全加固与并发一致性修复

本版无新增对外接口，集中修复登录/账号安全、并发写一致性、监控统计准确性与升级回滚可靠性。升级后数据库自动完成一次性结构迁移，无需操作。

### 安全
- 登录加固：无论邮箱是否存在、是否被封禁或锁定，密码错误一律返回统一「邮箱或密码错误」，并消除「存在账号才多走一次数据库查询/写入」造成的响应耗时差异，杜绝据此枚举账号是否存在。封禁/锁定状态仅在密码验证通过后才告知本人。
- 防账号锁定型攻击：账户已被多次失败登录锁定时，继续提交错误密码不再无限延长解锁时间，锁定封顶为单个固定的 15 分钟自动解锁窗口。
- 密码重置成功后自动清除账号锁定状态，被锁用户重置密码即可立即恢复登录，无需再等锁定窗口自然过期。
- 设备封禁加固：被封禁的 Web 用户即便丢弃或伪造设备 ID、仅保留同一强指纹，也仍会被指纹维度的封禁拦截，无法靠清空设备 ID 头绕过。
- 代理信任收口：客户端审计 IP、归属国家（GeoIP）、资源包下载地址、密码找回审计等，仅在开启「信任前置代理」（`trust_proxy`）时才采信 `X-Real-IP`/`X-Forwarded-For`/`X-Forwarded-Host`/`X-Forwarded-Proto` 等可伪造请求头；直连暴露时一律以真实连接来源为准，防止伪造头污染审计数据或将资源包下载地址劫持到恶意主机。生产环境建议直接设置 `RESOURCE_PACK_BASE_URL`。
- 版本门控防绕过：原生客户端（iOS/Android）若缺失或伪造客户端标识（UA）导致版本无法解析，在已配置最低版本时一律按「版本过低」硬拒绝，不再因丢改 UA 而逃过发布切流的强制升级门（Web 端不走该门，豁免）。
- 词书中心下载计数回调改为复用与主拉取一致的地址校验、地址固定与禁重定向策略，防止借用户可控来源地址经 DNS 重绑定/重定向打向内网。
- 学习配置选词收紧归属校验：仅系统词书或本人词书可选入每日学习，杜绝把他人私有词书 ID 写入越权读取。
- 管理后台「全部设备」CSV 导出对设备上报字段（如应用版本号）做公式注入防护，避免以 `=`/`+`/`-`/`@` 开头的内容被 Excel/WPS 当作公式执行。
- 数据导出（GDPR）冷却改为「检查+记录」单事务原子完成，关闭两个并发请求同时通过冷却门导致重复导出的窗口；下载因服务端故障中断时回滚本次冷却配额，而客户端主动断连则保留配额，防止反复开流绕过 24 小时限额。
- 账号注销与数据擦除更彻底：注销时一并清理磁盘上的头像文件及更多含个人数据的历史/审计表，消除注销后仍可被匿名访问的个人数据残留。

### 修复
- 并发写一致性：改密、改用户名、偏好设置、单词学习状态、学习配置、单词编辑等读改写路径改为字段级/单事务更新，修复并发请求互相用旧快照整行覆盖造成的丢更新（例如改密恰好覆盖掉管理员刚下发的封禁）。
- 每日新词不再卡住：修复学习进度推进后仍反复下发每本词书最靠前的若干个词、用户始终学不到后续生词的问题；同时按日历日与全时段已学正确去重，高频用户单日学习超量也不再重复下发已学词。
- 连续学习天数（streak）按应用默认时区（Asia/Shanghai）统计，修复东八区用户在本地凌晨学习被算到前一天、连续天数被低估、徽章延迟解锁。
- 监控指标修复：Prometheus 计数器恢复单调递增（不再每 5 分钟归零导致速率统计误判重启）；5xx 错误率告警修复进程冷启动期与低流量下的误报，并使告警声明的统计窗口与实际采样时长一致。
- 限流修复：双栈监听下来自 IPv4 客户端的请求不再被错误聚合到同一限流桶，恢复按真实 IPv4 独立计数（防单点拖垮全局限流/暴力破解防护退化）。跨域场景下客户端现可读到 `Retry-After` 响应头，获得精确重试时机。
- 头像上传：放宽对 JPEG 变体的识别（含带 ICC/Adobe 标记的合法图片），并调整写盘与数据库更新顺序、仅删除确切的上一份文件，避免并发换格式上传后头像地址指向已删除文件（404）。
- 升级/回滚可靠性：回滚落地改为原子替换现役数据库主文件并补充落盘同步（fsync），失败时现役库原封不动可恢复，不再出现中途崩溃留下空库或半截文件；旧版本备份清理修复在文件时间不可读时误删最新备份。
- 后台任务韧性：事件外发（outbox）死信/重排写入失败时不再静默吞掉并陷入无退避的繁忙重试回路，改为退避后下次重试；摄取拒绝留痕表与资源包安装留痕表纳入按保留期自动清理，避免只增不删持续膨胀。
- 维护模式/实时连接：实时连接（SSE）建立即下发当前维护状态并提前订阅广播，避免连上即维护却收不到任何提示；非法设备 ID 不再登记进实时连接与心跳，消除误报与卡死的心跳项。
- 管理后台界面修复：图表在数据系列减少时不再残留「幽灵」曲线/柱，空数据后到也能正常初始化；监控页离开后定时器泄漏已修复；资源包版本排序改为按版本号数值/字典序稳定排序；练习选项去除与正确答案重复的干扰项。
- 服务维护中、设备封禁等错误响应统一补齐 `success: false` 字段，便于客户端按统一约定解析；请求 ID 中间件在响应体读取失败或纯文本错误改写时不再返回带 JSON 头的空体或丢失时钟同步等响应头。

### 变更
- 数据导出（GDPR）「遗忘风险」明细仅返回风险最高的前 200 条，避免重度用户产生数 MB 级超大响应（汇总计数仍为全量）。
- AMAS 引擎配置新增边界校验：Elo 动态 K 值与趋势相关参数若超出合法区间将在启动时拒绝并报错，防止非法配置在学习热路径触发崩溃或数值发散。

## [v1.2.0-beta.24] — 2026-06-20 · Beta · 答题写入吞吐优化

### 变更
- 优化答题数据的写入路径，高并发上报下写入吞吐提升、延迟下降（同机压测吞吐约提升一半、中位延迟约减半）；学习记录越多的老用户提升越明显。算法决策与上报行为保持不变，升级后数据库自动完成一次性结构迁移、无需操作。
- 数据库预写日志（WAL）占用改为周期性自动回收，避免后端长期运行后该文件持续膨胀占用磁盘。

---

## [v1.2.0-beta.23] — 2026-06-20 · Beta · 探针跨管理员读隔离

### 安全
- 远程探针接口收紧越权读：非超级管理员现仅能查询 / 订阅自己下发的探针（脚本、目标设备、实时结果），不再能读取其他管理员下发的探针；探针下发与确认仍限超级管理员。

### 修复
- 探针采集错误率统计对单行错误计数做上下界钳制，避免设备上报异常值污染统计。
