  ...
  return (
    <Card variant="elevated" padding="lg">
      <div class="flex items-start gap-4">
        <div class={`w-10 h-10 rounded-lg flex items-center justify-center ${colors().bg}`}>
        ...
        <div class="flex-1 min-w-0">
          <p class={`text-3xl font-bold ${colors().text}`}>{props.value}</p>
          <p class="text-sm text-content-secondary">{props.title}</p>
          <Show when={trendDisplay()}>
            {(t) => (
              <p class={`text-xs mt-1 ${t().class}`}>
