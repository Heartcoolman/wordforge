 ...
 *   2. 同步采集 ctx 快照（M1 最小集，M2 扩到完整白名单）；
 *   3. spawn dedicated Worker 跑 admin 下发的 script，3s timeout 强制 terminate；
 *   4. JSON 序列化 result，超 256KB 截断 + truncated=true；
 *   5. `POST /api/probe/results` 回传。
 */

import { api, connectSseStream } from '@/api/client';
import { CLIENT_CTX_VERSION, type MinimalCtx, type ResultPayload, type WorkerInput, type WorkerOutput } from './types';
...
async function handleProbeRequest(payload: {
  ...
  const input: WorkerInput = { script, ctx };
  worker.postMessage(input);

  // 清理 timer 句柄不阻塞 onResult 路径，settled 标志已保护
  void timer;
}
