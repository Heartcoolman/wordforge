...
## [v1.2.0-beta.3] — 2026-06-16 · Beta · 管理后台新手引导对齐重设计

承接 beta.2 的管理后台蓝玻璃重设计，更新后端内嵌「新功能导览」(onboarding)，使其介绍本次重设计，而非旧版 v1.1.4 运营闭环功能。纯前端（运维面板）发布，服务端核心无逻辑变更，仅版本号 bump（core 1.2.0-beta.2 → 1.2.0-beta.3），无破坏性变更。

### 变更
- **新手引导重写**：欢迎屏版本号与亮点更新为 v1.2.0 重设计（蓝玻璃界面 / ⌘K 命令面板 / 暗色主题 / 自动刷新更顺滑 / 移除 echarts+CodeMirror 更轻量）；功能屏由旧 5 屏（设备遥测操作概览 / 死信运维 / 广播队列 / 备份 / 收件箱）精简重写为 3 屏（全新仪表盘 / 玻璃侧栏 7 组导航 / 自动刷新更顺滑），合计 5 屏。
- **导览波次修复**：`waveOf()` 此前把 1.2.x 误并入旧波 `v1.1-1.2`，导致 1.2 用户看到的是 v1.1.4 文案。现为 1.2.x 设独立波次 `v1.2-redesign`——看过旧导览的用户升级后会重新弹出一次新导览（同波内各 beta/patch 不重复弹）；同步更新单测与 e2e 预置波次值。
