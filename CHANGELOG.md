...
**这是自 v1.1.4（2026-06-05）以来首个实际发布的正式稳定版**，内容与 `v1.3.0-beta.2` 完全一致，覆盖 v1.1.4 以来的全部变更（注：`v1.2.0` 与 `v1.2.0-beta.29` 当时仅记入日志、未实际发布，其内容随本版一并进入正式版）。逐项明细见下方各 Beta 条目。

### 新增
- 三端统一客户端埋点事件流：`POST /api/telemetry/app-events` 批量上报行为 / 错误 / 性能事件，幂等去重、按日聚合。
- 埋点分析接口 `GET /api/admin/app-events/*` 共 8 个：总览、趋势、事件排行、错误分组、耗时分位、使用漏斗、留存矩阵、活跃度。
- 埋点独立采样开关（采样配置新增 `app_behavior` / `app_perf` 两行，错误类恒不采样）。
- 内置托管学习端 Web：学习端随服务端二进制在根路径托管，支持在线下发更新。
- 公开健康面板（`/health` 内容协商）与服务体征页。
- Web 设备指纹封禁与关联风控（精确自动封 + 模糊关联打标）。
- 管理后台 ⌘K 命令面板、暗色主题、新功能导览。
- 监控看板（look）全量服务端数据支撑：细粒度数据接口、拒绝码留痕、决策↔日志 `request_id` 关联。

### 变更
- 学习记录上报默认异步受理：`RECORDS_OUTBOX_ASYNC` 默认 `true`，单条上报返回 202，outbox 重试 + 死信兜底；设 `false` 可回退同步老路。
- 管理界面由根路径迁至 `/admin`，完成单色「黑墨白纸」重设计。
- 资源包安装结果上报细分五态：新增 `download_failed` / `apply_failed`，不再塌缩进 `verify_failed` 污染验签失败信号。
- Web 资源包支持显式回滚与回滚钉住。
- 自更新回滚重做：任意版本可回退、数据可恢复（pre-rollback 备份 + 降级引擎）。

### 优化
- AMAS 记忆引擎：FSRS 口径调参、真半衰期评估、学习调度退役机制、算法船值更新。
- 答题写入吞吐优化。
- 管理后台轮询页自动刷新不再跳顶 / 闪烁，移除大体积图表依赖、加载更快。

### 修复
- QuickPractice 记录幂等键修复，杜绝重复计入。
- due-count 统计返回未分页真实总数；状态字段大小写口径统一。
- 资源包删除接口保护范围收窄。
- 跨端契约审查 102 项发现的服务端部分全量清偿，含一批并发一致性修复。

### 安全
- 登录防枚举、代理信任收口、版本门控防绕过、GDPR 导出原子冷却、探针跨管理员读隔离。
- Web 设备指纹算法修正（去除不稳定维度），存量指纹全量清零、下次上报以新算法重建。
- LLM 调参白名单补冷启动难度先验权重，修复存量库静默拒绝已生效参数调参建议的问题。
