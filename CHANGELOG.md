...
**这是自 v1.1.4（2026-06-05）以来首个实际发布的正式稳定版**，内容与 `v1.3.0-beta.2` 完全一致。注：下方 `v1.2.0` 与 `v1.2.0-beta.29` 两条当时仅记入日志、未实际打 tag 发布，其内容随本版一并首次进入正式发布；从 v1.1.4 升级到本版会一次性获得以下全部变更。

### 累积范围一：1.2.x 主线（v1.2.0-beta.1 ~ beta.29，逐项明细见下方各 Beta 条目）

- **AMAS 记忆引擎**：FSRS 口径调参 + 真半衰期评估 + 学习调度退役机制 + 算法船值更新。
- **内置托管学习端 Web**：学习端随服务端二进制在根路径托管，支持 Web 端在线下发更新（`web-app` 签名工件管线）。
- **管理界面迁至 `/admin`** 并完成单色「黑墨白纸」重设计；⌘K 命令面板 / 暗色主题 / 顺滑自动刷新。
- **公开健康面板**（`/health` 内容协商）与服务体征页。
- **Web 设备指纹封禁与关联风控**（精确自动封 + 模糊关联打标）。
- **自更新回滚重做**：任意版本可回退 + 数据可恢复（pre-rollback 备份 + down 迁移降级引擎）。
- **安全加固**：登录防枚举 / 代理信任收口 / 版本门控防绕过 / GDPR 导出原子冷却 / 探针跨管理员读隔离，及一批并发一致性修复。
- **答题写入吞吐优化**、监控看板（look）全量服务端支撑（细粒度数据接口 / 拒绝码留痕 / 决策↔日志 request_id 关联）。

### 累积范围二：7 月中审查清偿批（此前未入日志，本版首次披露）

- **修复**：QuickPractice 记录幂等键修复（防重复计入）；作答事件崩溃安全持久化的服务端配套（due-count 返回未分页真实总数、状态字段大小写口径统一）；资源包删除接口保护范围收窄；跨端契约审查 102 项发现全量清偿中的服务端部分。
- **变更**：资源包安装结果上报细分五态（新增 `download_failed` / `apply_failed`，不再塌缩进 `verify_failed` 污染验签失败信号）；Web 资源包支持显式回滚与回滚钉住。
- **安全**：Web 设备指纹算法修正（去除不稳定维度），存量指纹全量清零、下次上报以新算法重建；LLM 调参白名单补冷启动难度先验权重（此前存量库会静默拒绝已生效参数的调参建议）。

### 累积范围三：1.3.0 新增（v1.3.0-beta.1 ~ beta.2）

- **三端统一客户端埋点事件流**：`POST /api/telemetry/app-events` 批量摄取（行为/错误/性能命名事件，幂等去重）+ 日聚合 + `GET /api/admin/app-events/*` 8 个分析读端点（总览/趋势/排行/错误分组/耗时分位/漏斗/留存矩阵/活跃度）+ 独立采样开关（`app_behavior` / `app_perf`）。
- **学习记录上报默认异步化**：`RECORDS_OUTBOX_ASYNC` 默认 `true`，单条上报 202 受理，outbox 重试 + 死信兜底 + 幂等账本。
- **管理后台新功能导览 v1.3 波次**。

### ⚠️ 升级前必读（从 v1.1.4 升级务必逐条确认）

1. **先升级学习端客户端再升级服务端**：iOS / Android / Web 须 ≥ 1.6.0（已发布）。旧客户端在记录异步受理模式下不崩溃，但每题即时反馈退化为估算/延迟回填；如需暂回同步老路设 `RECORDS_OUTBOX_ASYNC=false`。
2. **先手动备份数据库再升级**：Admin「版本更新 → 备份」手动触发一次，或 SSH 备份 `data/learning.db`。
3. **迁移单向不可逆**：自 v1.1.4 起累积多批数据库迁移（含指纹存量清理、监控数据回填、m073 埋点四表），升级成功后库结构前滚至 schema v73；回退仅允许换回旧二进制，**禁止**恢复旧库备份。
4. **管理界面入口变更**：管理后台由根路径迁至 **`/admin`**，根路径让位给内置学习端 Web；书签与反代规则需同步更新。
5. **反代部署检查 `RESOURCE_PACK_TRUSTED_HOSTS`**：开启 `trust_proxy` 且依赖转发主机推断资源包下载地址的部署，须设置该白名单（或直接设 `RESOURCE_PACK_BASE_URL`），否则下载地址回退为真实连接来源。
...
## [v1.2.0] — 2026-07-21 · Stable · 1.2.x 大版本转正（未实际发布；内容随 v1.3.0 首次进入正式版）
