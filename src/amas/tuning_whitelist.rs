...
/// 校验 patch 中所有 path / value 都通过白名单 + 范围检查。
/// 返回错误描述 vec；为空表示通过。
pub fn validate_patch(patch: &serde_json::Map<String, serde_json::Value>) -> Vec<String> {
    let mut errors = Vec::new();
    for (path, value) in patch {
        let Some(entry) = find(path.as_str()) else {
...
mod tests {
    ...
    fn validate_accepts_in_range_known_paths() {
        ...
        let errs = validate_patch(patch.as_object().unwrap());
