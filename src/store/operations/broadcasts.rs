...
impl Store {
    ...
    pub fn list_broadcasts_30d(
        ...
        let base: Vec<(String, String, String, String, i64, String)> = stmt
            .query_map(params![since_rfc3339, limit, offset], |r| {
                Ok((
                    r.get(0)?,
                    r.get(1)?,
                    r.get(2)?,
    ...
    pub fn broadcast_stats_30d(
        &self,
        since_rfc3339: &str,
    ) -> Result<(i64, i64, f64), StoreError> {
        let conn = self.conn()?;
        ...
        let ids: Vec<String> = {
            let mut stmt = conn.prepare(
                "SELECT id FROM broadcasts WHERE created_at >= ?1",
            )?;
            let ids = stmt
...
mod tests {
    ...
    fn record_and_list_with_read_count() {
        ...
        let rows = store.list_broadcasts_30d(since, 50, 0).unwrap();
    ...
    fn record_is_idempotent_replace() {
        ...
        store.record_broadcast(bid, "v1", "m", "adm", 1, &now).unwrap();
        store.record_broadcast(bid, "v2", "m", "adm", 5, &now).unwrap();
        let rows = store
            .list_broadcasts_30d("1970-01-01T00:00:00+00:00", 50, 0)
            .unwrap();
        assert_eq!(rows.len(), 1);
