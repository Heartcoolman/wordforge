...
impl Store {
    ...
    /// 全局单词记忆强度：跨所有用户按 word_id 聚合（avg 掌握度/半衰、sum 次数、用户数、avg MDM
    /// 记忆强度、众数状态），JOIN words 取词文本，按 avg 掌握度降序取 top-N。avgRetention 以
    /// MAX(updated_at) 近似末次复习 + 半衰指数衰减（2^(-Δh/half_life)，与 analytics 同模型）。
    pub fn aggregate_word_mastery(
        &self,
        limit: usize,
    ) -> Result<Vec<WordMasteryAggregate>, StoreError> {
        let limit_i = limit.clamp(1, 200) as i64;
        let now = Utc::now();
        let conn = self.conn()?;
        let mut stmt = conn.prepare(
            "SELECT wls.word_id, w.text,
                    AVG(wls.mastery_level), AVG(wls.half_life),
                    SUM(wls.total_attempts), COUNT(*),
                    AVG(ms.mdm_memory_strength), MAX(wls.updated_at),
                    (SELECT s2.state FROM word_learning_states s2
                     WHERE s2.word_id = wls.word_id
                     GROUP BY s2.state ORDER BY COUNT(*) DESC LIMIT 1)
             FROM word_learning_states wls
             JOIN words w ON w.id = wls.word_id
             LEFT JOIN mastery_states ms
                 ON ms.user_id = wls.user_id AND ms.word_id = wls.word_id
             GROUP BY wls.word_id, w.text
             HAVING SUM(wls.total_attempts) > 0
             ORDER BY AVG(wls.mastery_level) DESC, SUM(wls.total_attempts) DESC
             LIMIT ?1",
        )?;
        let items = stmt
            .query_map([limit_i], |r| {
                let avg_mastery: f64 = r.get(2)?;
                let avg_half_life: f64 = r.get(3)?;
                let last_updated: String = r.get(7)?;
                let state_str: String = r.get(8)?;
                let dominant_state = WordState::from_str(&state_str).map_err(|e| {
                    rusqlite::Error::FromSqlConversionFailure(
                        8,
                        rusqlite::types::Type::Text,
                        Box::new(e),
                ...
                let elapsed_h =
                    (now - parse_dt(last_updated)?).num_seconds().max(0) as f64 / 3600.0;
                let hl = avg_half_life.max(0.01);
                let avg_retention = 2f64.powf(-elapsed_h / hl).clamp(0.0, 1.0);
                Ok(WordMasteryAggregate {
                    word_id: r.get(0)?,
                    word_text: r.get(1)?,
                    avg_mastery,
                    avg_half_life,
                    total_attempts: r.get(4)?,
                    user_count: r.get(5)?,
                    avg_memory_strength: r.get::<_, Option<f64>>(6)?,
                    dominant_state,
                    avg_retention,
                })
            })?
...
/// 全局单词记忆强度聚合行（跨所有用户，看板 Amas WordMastery 表）。
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WordMasteryAggregate {
    pub word_id: String,
    ...
    pub user_count: i64,
    pub avg_memory_strength: Option<f64>,
    pub dominant_state: WordState,
