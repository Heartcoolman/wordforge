...
export const authApi = {
  ...
  forgotPassword: (email: string) =>
    api.post<{ emailSent: boolean; message: string }>('/api/auth/forgot-password', { email }),
  resetPassword: (token: string, newPassword: string) =>
    api.post<Record<string, never>>('/api/auth/reset-password', { token, newPassword }),
  verifyResetToken: (token: string) =>
    api.post<{ valid: boolean }>('/api/auth/verify-reset-token', { token }),
};
