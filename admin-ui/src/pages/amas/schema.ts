...
export const PARAM_DICT: Record<string, ParamMeta[]> = {
  featureFlags: [
    ...
    { path: 'featureFlags.confusionIsolationEnabled', label_zh: '启用混淆隔离排程', type: 'bool', default: false, description_zh: '同会话惩罚已出现词的高分混淆对端（Phase 1b）；需 wordSelector.confusionIsolationDampen < 1 才实际生效' },
    { path: 'featureFlags.multiTraceEnabled', label_zh: '启用跨题型多痕迹', type: 'bool', default: false, description_zh: 'mastery 按 question_mode 分痕迹、选词按 min-recall 聚合（Phase 2）' },
  ],
  ensemble: [
  ...
  memoryModel: [
    ...
    { path: 'memoryModel.alphaRampTau', label_zh: '信任调度·成功腿 τ_s', type: 'number', min: 0, max: 100, step: 0.5, default: 0, description_zh: '双腿信任调度成功腿：成功复习时残余阻尼 (1-α) 随当前连击数指数衰减（失败清零重启），连击越长越信任新证据；0 = 关闭（冻结语义）' },
    { path: 'memoryModel.alphaLapseRampTau', label_zh: '信任调度·失败腿 τ_f', type: 'number', min: 0, max: 100, step: 0.5, default: 0, description_zh: '双腿信任调度失败腿：失败（Again）时残余阻尼 (1-α) 随累计失败数指数衰减，首次失败不生效（偶发失误保护）、反复失败加速压低稳定性；0 = 关闭（冻结语义）' },
