...
const WAVE = 'v1.2-redesign';  // 当前波次（v1.2.x 管理后台蓝玻璃重设计）
const PRIOR_WAVE = 'v1.1.4r2'; // 上一波（v1.1.4 运营闭环收口）
const TOTAL = 5;               // STEPS 步数（welcome + 3 重设计屏 + outro）

function renderTour() {
  const ctl = useOnboarding();
  // 模拟 AdminLayout 注入当前版本，设好 currentWave 供 markSeen 写回
  shouldAutoShow('1.2.0-beta.3');
...
describe('useOnboarding 波次逻辑', () => {
  ...
  it('waveOf：1.2.x→v1.2-redesign；v1.1.4 起 v1.1.4r2；v1.1.3 独立；1.1.0–1.1.2 旧波；1.3+ 各自波', () => {
    expect(waveOf('1.2.0-beta.3')).toBe(WAVE);
    expect(waveOf('1.2.0')).toBe(WAVE);
    expect(waveOf('1.1.4-beta.3')).toBe(PRIOR_WAVE);
    expect(waveOf('1.1.4')).toBe(PRIOR_WAVE);
  ...
  it('未看过本波时 shouldAutoShow 返回 true；拿不到版本返回 false', () => {
    expect(shouldAutoShow('1.2.0-beta.3')).toBe(true);
    expect(shouldAutoShow(null)).toBe(false);
  });

  it('看过上一波(v1.1.4r2)升到 1.2.x 仍重弹；同波不再弹；跨大版本仍弹', () => {
    shouldAutoShow('1.1.4'); // 设 currentWave=v1.1.4r2
    markSeen();
    expect(localStorage.getItem(STORAGE_KEY)).toBe(PRIOR_WAVE);
    // 升到 1.2.0 → 新波 v1.2-redesign，重弹一次
    expect(shouldAutoShow('1.2.0-beta.3')).toBe(true);
    markSeen();
    expect(localStorage.getItem(STORAGE_KEY)).toBe(WAVE);
    // 同 v1.2-redesign 波内重复升级 → 不弹
...
describe('OnboardingTour', () => {
  ...
  it('渲染首屏标题（欢迎屏）', async () => {
    ...
    expect(screen.getByText('WordForge Admin v1.2.0')).toBeInTheDocument();
    expect(screen.getByText(`1 / ${TOTAL}`)).toBeInTheDocument();
  });

  it('点下一步切到第二屏（全新仪表盘）', async () => {
    renderTour();
    await screen.findByText('WordForge Admin v1.2.0');
    fireEvent.click(screen.getByRole('button', { name: '下一步' }));
    expect(await screen.findByText('全新仪表盘')).toBeInTheDocument();
